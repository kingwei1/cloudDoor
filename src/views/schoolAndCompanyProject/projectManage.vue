	...
	<div id="main">	
		<div class="backLayout">
			<el-breadcrumb separator-class="el-icon-arrow-right">
			  ...
			  <router-link :to="{path:'/addProject',query:{state:'add'}}"><el-button type="primary">创建项目</el-button></router-link>
			</el-breadcrumb>	
      <search :searchData="searchData" @search='search'></search>
			<tableList :tableData="tableData" :titles="titles" operate='true' flag='projectManage'   @open="open"></tableList>
      <pagination ref="page" :paginationShow="paginationShow" :listTotal="listTotal" @pageChangeSize="pageChangeSize" @page="page"></pagination>
			<dialogBox :dialogVisible='show' :flag="flag" :datas="datas" @close="close" @submitChange='submitChange'></dialogBox>
		</div>
	</div>
</template>
<script>
  ...
  import dialogBox from '../components/dialogBox'
	export default{
		...
		methods:{
			...
			submitChange(){
				console.log(this.datas.tableData2)
				let data = [];
				for(let i=0;i<this.datas.tableData2.length;i++){
					console.log(this.datas.tableData2[i].value);
					if(this.datas.tableData2[i].value == '' || this.datas.tableData2[i].value == undefined){
						this.$message.error('请确认所有人员的评价都填写完毕！');
						return;
					}else{
						data.push({
							id:this.datas.tableData2[i].id,
							evaluation:this.datas.tableData2[i].value
						})
					}
				}
				  console.log(data)
					...
					this.$axios.post(this.$schoolCompanyUrl+'/enterprise/evaluations/'+this.projectID+'?token='+token,data)
					.then(function(res){
							console.log(res);	
							if(res.data.code == 0){
								that.show=false;
					  ...
					  this.projectID = id;
						...
						.then(function(res){
								...
								if(res.data.code == 0){
									...
									if(flag == '完成项目'){
										that.datas.tableData2 = [];
										data.forEach(function(value,index,array){
											that.datas.tableData2.push({
													id:value.id,
      ...
      this.show=false;
			...
			getList(page){
				console.log(this.searchTime1);
				console.log(this.searchTime2);
				let that=this;
				const token=localStorage.getItem('token');
				let url = '';
				if(this.searchTime1 == '' || this.searchTime1 ==undefined){
					url = this.$schoolCompanyUrl+'/enterprise/projects?token='+token+'&pageNum='+page+'&pageSize='+this.pageSize+'&query='+this.searchInput+'&status='+this.searchState;
				}else{
					url = this.$schoolCompanyUrl+'/enterprise/projects?token='+token+'&pageNum='+page+'&pageSize='+this.pageSize+'&query='+this.searchInput+'&status='+this.searchState+'&startTime='+this.searchTime1+'&endTime='+this.searchTime2;
				}
				this.$axios.get(url)
				.then(function(res){
						console.log(res);
