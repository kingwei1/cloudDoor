    ...
    <div id="main">
        <div class="backLayout">
            <el-breadcrumb separator-class="el-icon-arrow-right">
                <el-breadcrumb-item :to="{ path: '/informationManage' }">资讯管理</el-breadcrumb-item>
                <el-breadcrumb-item>{{edit}}资讯</el-breadcrumb-item>
            </el-breadcrumb>
            <div class="box">
                <div class="item"><span>资讯名称</span><span><el-input v-model="datas.title"/></span></div>
                <div class="item">
                    <span>状态</span>
                    <div class="radio">
                ...
                <div class="item">
                    <span>资讯分类</span>
                    <div class="select">
                        <el-select v-model="datas.value" placeholder="请选择">
                            <el-option
                ...
                <div class="item"><span>资讯封面</span>
                    <span>
                    <upload  ref="upload" @changeImg="changeImg" flag='nobtnOnly'></upload>
                    </span>
                </div>
                <div class="item">
									<span>资讯描述</span>
									<span><el-input v-model="datas.description" placeholder="请输入描述" type="textarea" :rows="5" :maxlength="200"/></span>
								  <span style="margin-left: 20px;">{{datas.description.length}}/200</span>
								</div>
                <div class="item"><span>资讯内容</span><span><quill-editor ref="myTextEditor"
                                                          v-model="datas.EditorContent"
                                                          @blur="onEditorBlur($event)"
                                                          @focus="onEditorFocus($event)"
                                                          @ready="onEditorReady($event)">
                    </quill-editor></span></div>
    ...
    export default {
        ...
        methods:{
            ...
            submit(){
                const token=localStorage.getItem('token');
                let that=this;
                this.$refs.upload.upload(token).then((value)=>{ //上传封面后获取封面地址跟资讯信息一起传给后台
